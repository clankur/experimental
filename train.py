...
class TransformerLayer:
    ...
    w_q_gate: f32["n_partitions n_q_per_kv n_kv/t d_head/d"]
    w_k_routing: f32["n_partitions n_kv/t d_head/d"]
...
class Model:
    ...
    def init(h: Hparams, rng: PRNGKey) -> "Model":
        ...
        w_q_gate_shape = (h.layers, h.n_partitions, h.n_q_per_kv, h.n_kv, h.d_head)
    ...
    def forward_pass(
        ...
        def loop_body(
            ...
            w_k_routing = shardops.all_gather(
                "n_partitions K/t D/d -> n_partitions K/t D", layer_weights.w_k_routing
            )
            ...
            q_gate = shardops.einsum_unreduced(
                "B/d L Q K/t D, n_partitions Q K/t D -> B/d L Q K/t n_partitions",
                jax.lax.stop_gradient(nq),
                w_q_gate,
            )
            # key routing routes the each key to aspecific partition
            k_route = shardops.einsum_unreduced(
                "B/d L K/t D, n_partitions K/t D -> B/d L K/t n_partitions",
            ...
            relative_cluster_recall = jnp.where(
                total_probs > 0, cluster_recall / total_probs, 1
            )
            ...
            hard_k_c = jax.nn.one_hot(k_partition, h.n_partitions)
            hard_q_c = (
